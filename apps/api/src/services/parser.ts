...
export async function generateScopedSupportReply(params: {
  ...
  const prompt = [
    ...
    'Estrutura recomendada de comparação (curta): 1) validar a dúvida de preço/valor, 2) comparar foco do concorrente vs foco da Iara, 3) traduzir em benefício concreto para a rotina da pessoa (ex: menos esquecimento, mais clareza do mês, decisão mais rápida), 4) fechar com pergunta consultiva objetiva.',
    'Quando o cliente disser que está caro: responda com framing de valor (não defensivo), mostrando retorno prático no dia a dia e redução de desperdícios/erros, sem prometer economia com números inventados.',
    'Se perguntarem sobre quem criou a Iara: diga que foi idealizada por Felipe Grigoletti Guarde (dono), junto com a equipe de desenvolvimento, com foco em criar uma assistente financeira realmente útil no WhatsApp.',
    '',
